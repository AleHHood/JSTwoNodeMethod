...
import { pathArrowSVG } from "./constants";
...
const getCalculation = (branchs) => {
  ...
  function GetValueResist(R, K, numberBranch) {
    ...
    if (K < 2) {
      ...
      textArr.push(`tac g${R[K].number} =~1/${resistance}~=` + ` ${answer} См`);
      getAnswerBlock(textArr);
      textArr = [];
    } else {
      let Name = "",
  ...
  function Arrow(block, numberBranch, revers = 0) {
    ...
    if (!revers) {
      block.element.style.cssText = `background: url(${pathArrowSVG}) -130% -90% no-repeat;
  ...
  function GetVoltageSсheme() {
    ...
    textArr.push(
      `tac Uab =~${a}/${b}~=~${expressionEG}/${expressionG}~= ` +
        `${toFixed3(U)} В`
    );
    textArr.push(
  ...
  function getBalance(parameters) {
    ...
    parameters.forEach((element, i) => {
      ...
      expressionEI = expressionEI + ` ${plus}${element.voltage} ⋅ ${I[i]}`;

      b = b + ` + I${i}<sup>2</sup> ⋅ ${element.nameR}`;
      expressionRII =
        expressionRII + ` + ${I[i]}<sup>2</sup> ⋅ ${element.resistance}`;
    });
